  ...
  useEffect(() => {
    const loadQuizzes = async () => {
      try {
        ...
        const apiUrl = `${urlapi.backendurlsapi.quizzaccisible}/${studentId}`;
        ...
        if (studentId) {
          ...
          for (let i = 0; i < apiQuizzes.length; i++) {
            ...
            try {
              const attemptsResponse = await fetch(`${urlapi.backendurlsapi.attempts}?quiz_id=${quiz.id}&student_id=${studentId}`);
